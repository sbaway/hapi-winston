...
function logEvent(ctx, data, request) {
  try {
    ...
    if (obj.baseInfo) {
      if (ctx.level !== 'error') {
        ctx.log[ctx.level](`debug info is: ${JSON.stringify(obj)}`);
      } else {
        ctx.log[ctx.level](`error in request hapi server, error info is: ${JSON.stringify(obj)}`);
      }
    } else if (msg) {
      ctx.log[ctx.level](`error in request hapi server, error info is: ${msg}`);
    }
  } catch (e) {
